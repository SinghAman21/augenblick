...
# ── Fix 1: index.css — move @import to top ────────────────────────────────────
css_path = "src/index.css"
css = open(css_path, encoding="utf-8").read()
# Extract the Google Fonts import line
match = re.search(r"@import url\([^)]+\);", css)
if match:
    imp = match.group(0)
    css = css.replace(imp, "").lstrip("\n")
    css = imp + "\n\n" + css
    open(css_path, "w", encoding="utf-8").write(css)
    print("Fixed: index.css @import moved to top")
else:
    print("No @import found in index.css")

# ── Fix 2: SessionWorkspace.tsx — real ideas from Supabase ────────────────────
session_workspace = r"""import { useState, useEffect } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { AppLayout } from "@/components/app/AppLayout";
import { Share, Download, Plus, ThumbsUp, MessageCircle, Sparkles, Send, Lightbulb, Expand, Link2, FileText, GripVertical, Loader2, X } from "lucide-react";
...
export default function SessionWorkspace() {
  ...
  async function handleAddIdea() {
    ...
    setIdeas((prev) => [...prev, { ...data, votes: 0, comments: 0 }]);
    setNewTitle("");
    setNewDesc("");
    setSaving(false);
    setShowModal(false);
          ...
          <div className="flex-1 p-5 overflow-auto dot-grid">
            ...
            {loading && (
              <div className="flex items-center justify-center py-16">
                ...
                <span className="ml-2 text-sm text-muted-foreground">Loading ideas...</span>
              </div>
            )}
              ...
              <div className="flex-1 p-3 overflow-auto space-y-2">
                {aiMessages.map((msg, i) => (
                  <div key={i} className={"flex " + (msg.role === "user" ? "justify-end" : "justify-start")}>
                    <div className={"max-w-[90%] rounded-md px-3 py-2 text-[11px] " + (msg.role === "user" ? "bg-primary/15 border border-primary/20" : "bg-muted/40 border border-border/50")}>
                      {msg.text}
                    </div>
                  </div>
                ))}
...
path = "src/pages/SessionWorkspace.tsx"
os.makedirs(os.path.dirname(path), exist_ok=True)
with open(path, "w", encoding="utf-8") as f:
    f.write(session_workspace)
print("Written:", path)
print("All done.")
